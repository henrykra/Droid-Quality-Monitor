...
with rep.new_layer():
	...
	writer.initialize(output_dir="torso_output", rgb=True, bounding_box_2d_tight=True)
	writer.attach([render_product])
	
	# define randomizing function
	def torso():
		torso = rep.get.prims(path_pattern='/Replicator/Ref_Xform_01/Ref/droid/droid/torso')
		
		with torso:
			rep.modify.semantics([('class', 'torso_defect')])
			rep.randomizer.color(colors=random_defect_color())
		
		return torso


	rep.randomizer.register(torso)

	
	with rep.trigger.on_frame(max_execs=100):
		with droid:
			rep.modify.pose(rotation=rep.distribution.normal((0, 90, 0), (0, 10, 0)))
		rep.randomizer.torso()
