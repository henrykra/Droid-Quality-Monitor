...
with rep.new_layer():
	...
	writer.initialize(output_dir="left_arm_output", rgb=True, bounding_box_2d_tight=True)
	writer.attach([render_product])
	
	# define randomizing function
	def left_arm():
		left_arm = rep.get.prims(path_pattern='/Replicator/Ref_Xform_01/Ref/droid/droid/left_arm')
		
		with left_arm:
			rep.modify.semantics([('class', 'left_arm_defect')])
			rep.randomizer.color(colors=random_defect_color())
			rep.modify.pose(rotation=rep.distribution.normal((0, 0, 0), (10, 0, 0)))
		
		return left_arm


	rep.randomizer.register(left_arm)

	
	with rep.trigger.on_frame(max_execs=100):
		with droid:
			rep.modify.pose(rotation=rep.distribution.normal((0, 90, 0), (0, 10, 0)))
		rep.randomizer.left_arm()
