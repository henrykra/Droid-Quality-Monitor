...
with rep.new_layer():
	...
	writer.initialize(output_dir="legs_output", rgb=True, bounding_box_2d_tight=True)
	writer.attach([render_product])
	
	# define randomizing function
	def legs():
		legs = rep.get.prims(path_pattern='/Replicator/Ref_Xform_01/Ref/droid/droid/legs')
		
		with legs:
			rep.modify.semantics([('class', 'legs_defect')])
			rep.randomizer.color(colors=random_defect_color())
		
		return legs


	rep.randomizer.register(legs)
