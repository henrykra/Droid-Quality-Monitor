...
with rep.new_layer():
	...
	writer.initialize(output_dir="head_output", rgb=True, bounding_box_2d_tight=True)
	writer.attach([render_product])
	
	# define randomizing function
	def head():
		head = rep.get.prims(path_pattern='/Replicator/Ref_Xform_01/Ref/droid/droid/head')
		
		with head:
			rep.modify.semantics([('class', 'head_defect')])
			rep.randomizer.color(colors=random_defect_color())
		
		return head


	rep.randomizer.register(head)
